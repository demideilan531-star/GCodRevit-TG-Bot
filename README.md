...
Любое обычное текстовое или голосовое сообщение бот воспринимает как новую
задачу. Worker распознаёт формулировку, выделяет название, описание, срок и
флажки, а затем показывает черновик перед сохранением. Служебные команды и
кнопки Gmail, GitHub, видео и погоды продолжают выполнять свои сценарии.

Кнопка `Задачи` рядом со строкой ввода открывает Telegram Mini App со списком и
календарём. Она автоматически настраивается после `/start`. В приложении можно
создать или изменить задачу, отметить выполнение, отфильтровать записи по
